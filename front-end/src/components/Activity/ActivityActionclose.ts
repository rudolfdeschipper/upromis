/*
** Implementation of action Close (close) for Activity
...
export class ActivityActionclose {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IActivityData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IActivityData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Close", additionalData: [] };
