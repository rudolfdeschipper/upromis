/*
** Implementation of action Copy (copy) for Client
...
export class ClientActioncopy {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IClientData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IClientData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Copy", additionalData: [] };
