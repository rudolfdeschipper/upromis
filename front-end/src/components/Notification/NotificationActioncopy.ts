/*
** Implementation of action Copy (copy) for Notification
...
export class NotificationActioncopy {
    static perform = async (id: number, token: string): Promise<ISaveMessage<INotificationData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<INotificationData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Copy", additionalData: [] };
