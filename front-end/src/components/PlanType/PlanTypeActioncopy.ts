/*
** Implementation of action Copy (copy) for PlanType
...
export class PlanTypeActioncopy {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IPlanTypeData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IPlanTypeData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Copy", additionalData: [] };
