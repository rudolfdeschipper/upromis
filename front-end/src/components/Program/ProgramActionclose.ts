/*
** Implementation of action Close (close) for Program
...
export class ProgramActionclose {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IProgramData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IProgramData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Close", additionalData: [] };
