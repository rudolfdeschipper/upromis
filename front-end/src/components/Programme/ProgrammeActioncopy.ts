/*
** Implementation of action Copy (copy) for Programme
...
export class ProgrammeActioncopy {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IProgrammeData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IProgrammeData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Copy", additionalData: [] };
