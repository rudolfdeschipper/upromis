/*
** Implementation of action Close (close) for Workpackage
...
export class WorkpackageActionclose {
    static perform = async (id: number, token: string): Promise<ISaveMessage<IWorkpackageData>> => {
        try {
            ...
            var saveMessage : ISaveMessage<IWorkpackageData> =  { id: id, dataSubject : rec, action: "Save", subaction: "Close", additionalData: [] };
